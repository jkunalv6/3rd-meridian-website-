...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/contact' && request.method === 'POST') {
      ...
      await env.DB.prepare('INSERT INTO contact_submissions (name, email, message, created_at) VALUES (?, ?, ?, datetime(\'now\'))').bind(name, email, message).run();
    ...
    if (url.pathname === '/submissions' && request.method === 'GET') {
      ...
      const result = await env.DB.prepare('SELECT id, name, email, message, created_at FROM contact_submissions ORDER BY id DESC LIMIT ?').bind(limit).all();
